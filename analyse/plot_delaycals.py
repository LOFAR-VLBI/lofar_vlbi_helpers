...
fitsfiles = ['selfcal_allstations_LBCS_4sets_default_fullpol000-MFS-I-dirty.fits',
             'selfcal_allstations_LBCS_4sets_default_fullpol000-MFS-I-image.fits']
# fitsfiles = ['selfcal_allstations4_LBCS_final_fullpol000-MFS-I-dirty.fits',
#              'selfcal_allstations4_LBCS_final_fullpol000-MFS-I-image.fits']
...
for i, fitsfile in enumerate(fitsfiles):
    ...
    if 'dirty' in fitsfile.split('/')[-1]:
        n=0
        title='Dirty image'
    elif 'image' in fitsfile.split('/')[-1]:
        n=1
        title='Final image'
    else:
        continue
    ...
    if i==0:
        fig, axes = plt.subplots(figsize=(12, 7.5), nrows=1, ncols=2)
        rms = findrms(imdata)
        vmin, vmax = -0.1, 100 * rms

    im = axes[n].imshow(imdata, origin='lower', cmap=cmap, norm=SymLogNorm(linthresh=abs(vmin), vmin=vmin, vmax=vmax))
    axes[n].set_yticks([])
    axes[n].set_xticks([])
    axes[n].set_title(title, fontsize=16)
...
plt.savefig('delaycal.png', dpi=300)
plt.close()
