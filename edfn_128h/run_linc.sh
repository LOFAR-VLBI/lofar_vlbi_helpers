...
SIMG=/project/wfedfn/Software/singularity/flocs_v5.2.0_znver2_znver2.sif
...
# Ensure < 168 MHz
singularity exec -B ${SING_BIND} ${SIMG} python ~/scripts/lofar_vlbi_helpers/edfn_128h/download_scripts/removebands.py --freqcut 168 --datafolder data
...
# Run LINC target
singularity exec -B ${SING_BIND} ${SIMG} $FLOCSRUNNERS/run_LINC_target_HBA.sh -d $STARTDIR/target/data -c $STARTDIR/calibrator/*_LINC_calibrator/results_LINC_calibrator/cal_solutions.h5 -e "--make_structure_plot=False" -l /project/wfedfn/Software/LINC
