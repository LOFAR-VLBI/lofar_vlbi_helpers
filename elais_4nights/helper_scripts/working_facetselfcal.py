...
def check_code_is_uptodate():
    ...
    try:
        result = subprocess.run(["wget", "--no-cache", "--backups=1", url, "--output-document=tmpfile"],
        ...
        for ff in range(data.shape[1] - 1):
            if ampnorm:
                ...
                dataslope[:, ff, 0] = np.copy(
                    np.abs(data[:, ff, 0]) * np.exp(1j * (np.angle(data[:, ff, 0]) - np.angle(data[:, ff + 1, 0]))))
                dataslope[:, ff, 3] = np.copy(
                    np.abs(data[:, ff, 3]) * np.exp(1j * (np.angle(data[:, ff, 3]) - np.angle(data[:, ff + 1, 3]))))
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        raise Exception(
                            'No amplitude000,phase000, tec000, rotation000, or rotationmeasure000 solutions found')
    H.close()
    return len(times)
...
def getmsmodelinfo(ms, modelcolumn, fastrms=False, uvcutfraction=0.333):
    ...
    if fastrms:  # take only every fifth element of the array to speed up the computation
        if freq > freqct:  # HBA
            noise = astropy.stats.sigma_clipping.sigma_clipped_stats(
                data[0:data.shape[0]:5, np.int(np.floor(np.float(nfreq) * HBA_upfreqsel)):-1, 1:3], \
                mask=flags[0:data.shape[0]:5, np.int(np.floor(np.float(nfreq) * HBA_upfreqsel)):-1, 1:3])[
                2]  # use XY and YX
        else:
        ...
        if freq > freqct:  # HBA
            noise = astropy.stats.sigma_clipping.sigma_clipped_stats(
                data[:, np.int(np.floor(np.float(nfreq) * HBA_upfreqsel)):-1, 1:3], \
    ...
    if freq > freqct:  # HBA:
        flux = np.ma.mean((model[:, np.int(np.floor(np.float(nfreq) * HBA_upfreqsel)):-1, 0] + model[:, np.int(
            np.floor(np.float(nfreq) * HBA_upfreqsel)):-1,
        ...
        for soltype_id, soltype in enumerate(soltype_list):
                ...
                if not longbaseline and (tint * solint_sf * ((noise / flux) ** 2) * (chanw / 390.625e3) > tint):
                    ...
                    if (tint * solint_sf * ((noise / flux) ** 2) * (
                        ...
                        insoltypecycles_list[soltype_id + 1][ms_id] = insoltypecycles_list[soltype_id][
                            ms_id]  # do + 1 here??
                        insolint_list[soltype_id + 1][ms_id] = np.int(
            ...
            if soltype in ['complexgain', 'scalarcomplexgain'] and (
                ...
                if ((solint_sf * ((noise / flux) ** 2) * (
                    ...
                    logger.info('Setting gain solint to 20 min (the min value allowed): ' + str(
                        np.float(solint) * tint / 3600.))
...
def medianamp(h5):
    ...
    if fulljones:
        ...
        medamps_cross = 0.5 * (
                    10 ** (np.nanmedian(np.log10(amps_xy[idx_xy]))) + 10 ** (np.nanmedian(np.log10(amps_yx[idx_yx]))))
...
def prepare_DDE(imagebasename, selfcalcycle, mslist, imsize, pixelscale, \
                ...
                DDE_predict='DP3', restart=False, disable_IDG_DDE_predict=False, telescope='LOFAR', dde_skymodel=None,
                targetFlux=2.0, skyview=None):
...
def create_facet_directions(imagename, selfcalcycle, targetFlux=1.0, ms=None, imsize=None, \
                            pixelscale=None, groupalgorithm='tessellate', numClusters=10, weightBySize=False,
                            facetdirections=None):
    '''
   ...
   '''
    ...
    if facetdirections is not None:
        ...
        LSM.group(algorithm=groupalgorithm, targetFlux=str(targetFlux) + ' Jy', numClusters=numClusters,
        ...
        for msnumber, ms in enumerate(mslist):
            ...
            if selfcalcycle >= soltypecycles_list[soltypenumber][msnumber]:
                ...
                runDPPPbase(ms, solint_list[soltypenumber][msnumber], nchan_list[soltypenumber][msnumber], parmdb,
                            ...
                            DDE_predict=DDE_predict, telescope=telescope, ncpu_max=ncpu_max)
    ...
    for soltypenumber, soltype in enumerate(soltype_list):
        ...
        for msnumber, ms in enumerate(mslist):
            ...
            if selfcalcycle >= soltypecycles_list[soltypenumber][msnumber]:
                ...
                create_modeldata = True
                if soltypenumber >= 1:
                    for tmpsoltype in ['complexgain', 'scalarcomplexgain', 'scalaramplitude', 'amplitudeonly',
                                       'phaseonly', \
                                       'fulljones', 'rotation', 'rotation+diagonal', 'tec', 'tecandphase',
                                       'scalarphase', \
                                       'phaseonly_phmin', 'rotation_phmin', 'tec_phmin', \
                                       'tecandphase_phmin', 'scalarphase_phmin', 'scalarphase_slope',
                                       'phaseonly_slope']:
                        if tmpsoltype in soltype_list[0:soltypenumber]:
                            print('Previous solve already predicted MODEL_DATA, will skip that step', soltype,
                                  soltypenumber)
                            create_modeldata = False
                ...
                runDPPPbase(ms, solint_list[soltypenumber][msnumber], nchan_list[soltypenumber][msnumber], parmdb,
                            ...
                            clipsollow=args['clipsollow'], uvmax=args['uvmax'], modeldatacolumns=modeldatacolumns,
                            preapplyH5_dde=parmdbmergelist[msnumber], dde_skymodel=dde_skymodel,
                            DDE_predict=DDE_predict, ncpu_max=ncpu_max)
...
def runDPPPbase(ms, solint, nchan, parmdb, soltype, uvmin=0, \
                ...
                modeldatacolumns=[], solveralgorithm='directionsolve', solveralgorithm_dde='directioniterative',
                preapplyH5_dde=[], \
    ...
    if BLsmooth:
        print('python BLsmooth.py -n 8 -c ' + str(
            blsmooth_chunking_size) + ' -i ' + incol + ' -o SMOOTHED_DATA -f ' + str(iontimefactor) + \
              ' -s ' + str(blscalefactor) + ' -u ' + str(ionfreqfactor) + ' ' + ms)
        run('python BLsmooth.py -n 8 -c ' + str(
            blsmooth_chunking_size) + ' -i ' + incol + ' -o SMOOTHED_DATA -f ' + str(iontimefactor) + \
            ' -s ' + str(blscalefactor) + ' -u ' + str(ionfreqfactor) + ' ' + ms)
        incol = 'SMOOTHED_DATA'
    ...
    if soltype in ['complexgain', 'scalarcomplexgain', 'scalaramplitude', 'amplitudeonly', 'rotation+diagonal',
                   'fulljones']:
    ...
    if resetsols is not None:
        if soltype in ['phaseonly', 'scalarphase', 'tecandphase', 'tec', 'rotation', 'fulljones', 'complexgain',
                       'scalarcomplexgain']:
    ...
    if soltype in ['scalarcomplexgain', 'complexgain', 'amplitudeonly', 'scalaramplitude', 'fulljones',
                   'rotation+diagonal']:
    ...
    if soltype in ['rotation', 'rotation+diagonal']:
        losotoparset_rotation = create_losoto_rotationparset(ms, onechannel=onechannel, outplotname=outplotname,
                                                             refant=findrefant_core(parmdb))  # phase matrix plot
        force_close(parmdb)
    ...
    if soltype in ['scalarcomplexgain', 'complexgain', 'amplitudeonly', 'scalaramplitude', \
                   'fulljones', 'rotation+diagonal'] and (
...
def main():
    ...
    imagingparser.add_argument('--fitsmask',
                               help='Fits mask for deconvolution (needs to match image size). If this is not provided automasking is used.',
                               type=str)
    imagingparser.add_argument('--robust', help='Briggs robust parameter. The default is -0.5.', default=-0.5,
                               type=float)
    ...
    imagingparser.add_argument('--pixelscale', '--pixelsize',
                               help='Pixels size in arcsec. Typically, 3.0 for LBA and 1.5 for HBA (these are also the default values).',
                               type=float)
    ...
    imagingparser.add_argument('--multiscalescalebias',
                               help='Multiscalescale bias scale parameter for WSClean (see WSClean documentation). This is by default 0.7.',
                               default=0.7, type=float)
    imagingparser.add_argument('--usewgridder',
                               help='Use wgridder from WSClean, mainly useful for very large images. This is by default True.',
                               type=ast.literal_eval, default=True)
    ...
    imagingparser.add_argument('--fitspectralpol',
                               help="Use fit-spectral-pol in WSClean (see WSClean documentation). The default is True.",
                               type=ast.literal_eval, default=True)
    imagingparser.add_argument('--fitspectralpolorder',
                               help="fit-spectral-pol order for WSClean (see WSClean documentation). The default is 3.",
                               default=3, type=int)
    ...
    calibrationparser.add_argument('--autofrequencyaverage-calspeedup',
                                   help="Try extra averaging during some selfcalcycles to speed up calibration.",
                                   action='store_true')
    ...
    calibrationparser.add_argument("--preapplyH5-list", type=arg_as_list, default=[None],
                                   help="List of H5 files to preapply (one for each MS). The default is [None].")
    ...
    calibrationparser.add_argument('--ncpu-max-DP3solve',
                                   help='Maximum number of threads for DP3 solves, default=24 (too high value can result in BLAS errors)',
                                   type=int, default=24)
    calibrationparser.add_argument('--DDE', help='Experts only.', action='store_true')
    calibrationparser.add_argument('--Nfacets',
                                   help='Number of directions to solve into when --DDE is used. Directions are found automatically. Only used if --facetdirections is not set.',
                                   type=int, default=8)
    calibrationparser.add_argument('--targetFlux',
                                   help='targetFlux in Jy for groupalgorithm to create facet directions when --DDE is set (default = 2.0). Only used if --facetdirections is not set. Note that --targetFlux takes presidence over --Nfacets',
    ...
    flaggingparser.add_argument('--aoflagger-strategy',
                                help='Use this strategy for AOflagger (options are: "default_StokesV.lua")',
                                default=None, type=str)
    ...
    startmodelparser.add_argument('--startfromvlass',
                                  help='Start from VLASS skymodel for ILT phase-up core data (not yet implemented).',
                                  action='store_true')
    ...
    parser.add_argument('--remove-outside-center',
                        help='Subtract sources that are outside the central parts of the image, square box is used (experimental)',
                        action='store_true')
    ...
    parser.add_argument('--targetcalILT',
                        help="Type of automated target calibration for HBA international baseline data when --auto is used. Options are: 'tec', 'tecandphase', 'scalarphase'. The default is 'tec'.",
                        default='tec', type=str)
    ...
    if os.path.isfile(config):
        ...
        for line in lines:
            ...
            if arg not in vars(options).keys():
                raise KeyError('Encountered invalid option {:s} in config file {:s}.'.format(arg, os.path.abspath(
    ...
    t.close()
    idgin = args[
        'idg']  # store here as we update args['idg'] at some point to made image000 for selfcalcycle 0 in when --DDE is enabled
    ...
    for i in range(args['start'], args['stop']):
        ...
        if args['DDE'] and args['start'] != 0:  # set modeldatacolumns and dde_skymodel for a restart
            modeldatacolumns, dde_skymodel, candidate_solints = prepare_DDE(args['imagename'], i, \
                                                                            ...
                                                                            restart=True, \
                                                                            disable_IDG_DDE_predict=args[
                                                                                'disable_IDG_DDE_predict'],
                                                                            telescope=telescope, \
        ...
        if len(modeldatacolumns) > 1:
            ...
            if args['DDE'] and i == 0:  # we are making image000 without having DDE solutions yet
                if telescope == 'LOFAR' and not args[
                    'disable_IDG_DDE_predict']:  # temporarilly ensure idg=True so image000 has model-pb
                    args['idg'] = True
